...
\title{Concentration Inequalities Reference}
\author{Lily Chung}
\date{}
...
\begin{thmbox}[label=thm:markov]{Markov's inequality}
...
\iffalse
\begin{proof}
  Apply \cref{thm:markov} to $(X - \E X)^2$.
\end{proof}
\fi
...
\begin{thmbox}[label=thm:azuma]{Azuma-Hoeffding inequality}
  Let $M_0, \dots, M_n$ be a supermartingale with respect to the filtration $\mathcal F$,
  and let $A_i, B_i$ be $\mathcal F$-predictable processes such that $A_i \le |M_i - M_{i-1}| \le B_i$ and $B_i - A_i \le c_i$ a.s. for all $i$.
  Then
  \[\Pr[M_n \ge M_0 + t] \le \exp\left(-\frac{2t^2}{R}\right)\]
  for any $t \ge 0$, where $R = \sum_i c_i^2$.
\end{thmbox}
\begin{itemize}
\item Generalization of Hoeffding to martingale increments.
\item Can be used to bound partial sums like $\max_j \sum_{i=1}^j X_i$ by considering a stopped martingale.
\end{itemize}
...
\begin{corbox}{Chernoff bounds (looser versions)}
  ...
  \Pr[X \ge \E X \cdot t] &\le \exp\left(-\E X \cdot (Ct - e^C + 1)\right) &\text{for $t \ge 1, C \in \R$} \\
  ...
  \Pr[X \ge \E X \cdot t] &\le \exp\left(-\E X \cdot (Cs - e^C + 1)\right) &\text{for $s \in (0, 1), C \in \R$} \\
  \]
\end{corbox}
