...
\marginnote{Fubini-Tonelli} \int_{S\times S'} f\,d(\mu \times \mu') &= \int_S \int_{S'} f\,d\mu'\,d\mu & \text{provided left integral exists} \\
...
\E[f(X, Y)] &= \E[(y \mapsto \E[f(X, y)])(Y)] & X \Perp Y
\]

\subsection*{Inclusion-exclusion}

$A_1, A_2, \dots$ are measurable sets with finite measure.
Let $\#A$ be the number of $A_i$ which occur.

\[
\mu[\#A \ge 1] &= \lim_{n \to \infty} \sum_{\substack{I \subset n \\ 1 \le |I|}} (-1)^{|I|+1} \mu\left(\bigcap_{i \in I} A_i\right) \\
\marginnote{Bonferroni inequalities}
\mu[\#A \ge 1] &\le \sum_{\substack{I \subset \N \\ 1 \le |I| \le k}} (-1)^{|I|+1} \mu\left(\bigcap_{i \in I} A_i\right) &\text{$k$ odd} \\
\mu[\#A \ge 1] \ge \mu[1 \le \#A \le k] &\ge \sum_{\substack{I \subset \N \\ 1 \le |I| \le k}} (-1)^{|I|+1} \mu\left(\bigcap_{i \in I} A_i\right) &\text{$k$ even}
\]

\begin{proof}
  For $k \in \N$ define the measurable function
  \[f \eqdef \sum_{\substack{I \subset \N \\ 1 \le |I| \le k}} (-1)^{|I|+1} \left[\cap_{i \in I} A_i\right].\]
  Note that there are a countable number of terms in the sum,
  and at each point only finitely many of them are nonzero.

  We observe:
  \[
  f &= \sum_{i=1}^{k} (-1)^{i+1} \binom{\#A}{i} \\
  &= \binom{\#A - 1}{0} + (-1)^{k+1}\binom{\#A - 1}{k}.
  \]
  We find:
  \begin{itemize}
  \item If $\#A = 0$ then $f = 0$.
  \item If $1 \le \#A \le k$ then $f = 1$.
  \item If $\#A > k$ and $k$ is odd then $f \ge 1$.
  \item If $\#A > k$ and $k$ is even then $f \le 0$.
  \end{itemize}
  Consequently:
  \begin{itemize}
  \item If there are at most $k$ events then $f = [\#A \ge 1]$.
  \item If $k$ is odd then $f \ge [\#A \ge 1]$.
  \item If $k$ is even then $f \le [1 \le \#A \le k]$.
  \end{itemize}
  Integrating yields the inequalities
  and the equality with finitely many events.
  Applying monotone convergence completes the proof.
\end{proof}
...
X &\le {\psi_X^*}^{-1}(\ln \delta^{-1}) &\text{w.p. $\ol{\delta}$} \\
...
\E X &= np \\
\Var X &= np\overline{p} \\
\psi_X(\lambda) &= n \ln (\overline{p} + pe^\lambda) \\
\psi_{X - \E X}(\lambda) &= n (\ln (\overline{p} + pe^\lambda) - \lambda p) \\
&\le \psi_{\Poi(np\overline{p}) - np\overline{p}}(\lambda) \marginnote{\cite[Thm~2.9]{BouLugMas2013}} \\
&\le \psi_{\Poi(np) - np}(\lambda) \\
\psi_{X}^*(t) &= \frac{n}{\log e} d\left(\frac{t}{n} \mathrel{}\middle\|\mathrel{} p\right) & d(a \mathrel{\|} p) = a \log \frac{a}{p} + \ol{a}\log\frac{\,\ol{a}\,}{\,\ol{p}\,} \\
\]
...
\psi_X(\lambda) &= a(e^\lambda - 1) \\
\psi_X^*(t) &= ah\left(\frac{t}{a}\right) & h(u) = u \ln u + 1 - u \\
\]
