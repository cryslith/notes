...
For many applications in computer science we don't need to be concerned about the exact numerical constants,
so we will often ignore constant factors.  Instances of asymptotic notation ($O, \Theta$, etc.) are with respect to the inverse error probability $\delta^{-1}$.
...
\begin{theorem}[Chernoff]
  ...
  Then \[
  \Pr[X \ge t] &\le \exp\left(-n d\left(\tfrac{t}{n} \middle\| \tfrac{\mu}{n}\right)\right) & t \in [\mu, n] \\
  \Pr[X \le t] &\le \exp\left(-n d\left(\tfrac{t}{n} \middle\| \tfrac{\mu}{n}\right)\right) & t \in [0, \mu] \\
  \]
\end{theorem}

To apply the above we often use the inequality:

\[
n d\left(\tfrac{t}{n} \middle\| \tfrac{\mu}{n}\right) &\ge \mu h\left(\frac{t}{\mu}\right) & h(x) = x \ln x - x + 1
\]

which can be refined by a bound like

\[
h(x) &\ge 
\begin{cases}
  \frac{(x-1)^2}{2} & x \in [0, 1] \\
  \frac{(x-1)^2}{x+1} & x \ge 1 \\
  x & x \ge 6.4 \\
  2x & x \ge 19.1
\end{cases}
\]
