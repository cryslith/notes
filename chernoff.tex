...
% todo: subgauss, subexp, subgamma, subpoisson versions?
...
If we are only interested in asymptotic quantities, we can use the following rules:
...
\begin{tcolorbox}
  \begin{lemma}
    ...
    \item With high probability \(X \in O(\E X + \log n)\).
...
\begin{proof}\hfill
  ...
  \item
    Pick any $c \ge 1$, and set $u = \max\{\E X, c \log n\}$.
    Then we have
    \[\Pr[X \ge 3u] \le \exp(-u) \le \exp(-c\log n) = n^{-c}\]
